  ...
  if (!alphaPass) {
    return {
      ...
      reason: direction === 'SHORT'
        ? `Final Alpha ${alpha.toFixed(1)} is above the BUY PE limit of ${SHORT_ALPHA_MAX}.`
        : `Final Alpha ${alpha.toFixed(1)} is below the BUY CE threshold of ${LONG_ALPHA_MIN}.`,
      direction,
      action,
      alpha,
...
export function rankScore(result: FnoScanResponse): number {
  ...
  return alphaStrength(result) * 0.72
    + Math.min(optionRiskReward(result), 4) * 5
    + (Math.log10(1 + oi) + Math.log10(1 + volume)) * 2;
}
