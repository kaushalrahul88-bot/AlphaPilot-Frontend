...
async function requestJson<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  for (let attempt = 0; attempt < RETRY_DELAYS_MS.length; attempt += 1) {
    ...
    try {
      ...
      // A 429 means the upstream/provider quota is already exhausted. Retrying
      // immediately only adds more pressure and can extend the cooldown window.
      if (response.status === 429) {
        emitApiError(path, message);
        throw Object.assign(new Error(message), { noRetry: true });
      }

      if (response.status !== 408 && response.status < 500) throw Object.assign(new Error(message), { noRetry: true });
  ...
  const finalMessage = retryCount ? `${message} (retried ${retryCount} times)` : message;
