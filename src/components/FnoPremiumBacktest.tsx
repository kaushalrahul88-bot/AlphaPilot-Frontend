...
function candidateRows(result:PremiumBacktestResponse|null){
  const rows=result?.trades??[];
  const before1030=(t:PremiumBacktestTrade)=>{const h=tradeHour(t);return h!=null&&h<10.5};
  const notMidday=(t:PremiumBacktestTrade)=>{const h=tradeHour(t);return h!=null&&(h<10.5||h>=12)};
  const tests=[
    {label:'0.5R · Before 10:30',test:before1030},
    {label:'0.5R · BUY PE before 10:30',test:(t:PremiumBacktestTrade)=>t.action==='BUY PE'&&before1030(t)},
    {label:'0.5R · BUY CE before 10:30',test:(t:PremiumBacktestTrade)=>t.action==='BUY CE'&&before1030(t)},
    {label:'0.5R · Exclude 10:30–12:00',test:notMidday},
    {label:'0.5R · PE · Exclude 10:30–12:00',test:(t:PremiumBacktestTrade)=>t.action==='BUY PE'&&notMidday(t)},
  ];
  return tests.map(x=>({label:x.label,...subsetSummary(rows.filter(x.test),'0.5R')}));
}
...
export function FnoPremiumBacktest(){
  ...
  return <Card><CardHeader title="True F&O Premium Backtest" subtitle="Core strategy test: historical AlphaPilot signals replayed through actual Groww option-premium OHLC. No live rules are changed." action={<ShieldCheck size={18} className="text-emerald-500"/>}/><CardBody className="space-y-4">
    ...
    {result&&<>
      ...
      {hasTargetScenarios&&<div className="rounded-lg border border-slate-200 dark:border-slate-800 p-3"><div className="flex flex-wrap justify-between gap-2"><div><p className="text-sm font-semibold">Candidate combinations to validate next</p><p className="text-[11px] text-slate-500 mt-1">These combine only the strongest clues from this development sample. All use the exact 0.5R replay and are hypotheses, not live rules.</p></div><Badge variant="amber">DO NOT DEPLOY</Badge></div><div className="overflow-x-auto mt-3"><table className="w-full text-xs"><thead><tr className="border-b border-slate-200 dark:border-slate-800"><th className="text-left p-2">Candidate</th><th>Trades</th><th>Avg R</th><th>Win %</th><th>Total R</th></tr></thead><tbody>{candidates.map(c=><tr key={c.label} className="border-b border-slate-100 dark:border-slate-900"><td className="p-2 font-medium">{c.label}</td><td className="text-center">{c.trades}</td><td className={`text-center font-semibold ${c.avg>0?'text-emerald-600':c.avg<0?'text-red-600':''}`}>{c.avg>=0?'+':''}{c.avg.toFixed(2)}R</td><td className="text-center">{c.winRate.toFixed(1)}%</td><td className={`text-center ${c.total>0?'text-emerald-600':c.total<0?'text-red-600':''}`}>{c.total>=0?'+':''}{c.total.toFixed(2)}R</td></tr>)}</tbody></table></div><p className="text-[10px] text-slate-500 mt-2">Only a candidate that stays positive across a larger development window should be taken into an untouched out-of-sample test.</p></div>}
