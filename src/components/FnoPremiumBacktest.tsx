...
export function FnoPremiumBacktest(){
  ...
  async function run(){
    ...
    try{
      setResult(await runTruePremiumBacktest({symbols,start_date:start,end_date:end,expiry:autoExpiry?null:expiry,min_risk_reward:Number(rr)||1.5,entry_before:entryBefore||null,max_trades:Math.max(1,Math.min(Number(maxTrades)||20,50))}));
  ...
  return <Card><CardHeader title="True F&O Premium Backtest" subtitle="Core strategy test: historical AlphaPilot signals replayed through actual Groww option-premium OHLC. No live rules are changed." action={<ShieldCheck size={18} className="text-emerald-500"/>}/><CardBody className="space-y-4">
    <div className="grid grid-cols-1 md:grid-cols-7 gap-2 items-end">
      ...
      <div><label className="text-xs font-medium">Expiry mode</label><select className="block mt-1 w-full rounded-lg border border-slate-200 dark:border-slate-800 bg-transparent px-3 py-2.5 text-sm" value={autoExpiry?'AUTO':'FIXED'} onChange={e=>setAutoExpiry(e.target.value==='AUTO')}><option value="AUTO">Auto nearest expiry</option><option value="FIXED">Fixed expiry</option></select></div>
      {autoExpiry?<div className="rounded-lg border border-slate-200 dark:border-slate-800 px-3 py-2.5"><p className="text-xs text-slate-500">Expiry</p><p className="text-sm font-semibold">Auto per trade</p></div>:<Input label="Expiry" type="date" value={expiry} onChange={setExpiry}/>} 
    ...
    <p className="text-[11px] text-slate-500">Auto Expiry selects the nearest Groww-listed option expiry on or after each historical signal date. This lets one run cross expiry boundaries without forcing every trade into a single contract series. Contracts absent from Groww's current instrument master are reported as errors, never invented.</p>
