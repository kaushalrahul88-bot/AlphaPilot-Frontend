...
export function CommodityManagementSimulation(){
  ...
  const run=async()=>{setLoading(true);try{const symbols=Array.from(new Set(resolved.map(r=>r.symbol).filter(s=>s==='CRUDEOIL'||s==='NATURALGAS'))) as CommoditySymbol[];const feeds=new Map<string,any>();await Promise.all(symbols.map(async s=>{try{feeds.set(s,await getCommodityCandles(s,'5m'))}catch{feeds.set(s,null)}}));let mismatch=0,history=0;const baselineResults:SimResult[]=[],fullHalf:SimResult[]=[],partialHalf:SimResult[]=[],beHalf:SimResult[]=[],beOne:SimResult[]=[];for(const r of resolved){const feed=feeds.get(r.symbol);if(!feed){history++;continue;}const returned=String(feed?.contract?.trading_symbol??feed?.contract?.groww_symbol??'');if(r.contract&&returned&&r.contract!==returned){mismatch++;continue;}const cs=Array.isArray(feed?.candles)?feed.candles:[];if(!windowCandles(r,cs).length){history++;continue;}const base=baseline(r);baselineResults.push(base);fullHalf.push(fullHalfR(r,cs));partialHalf.push(partialHalfR(r,cs));beHalf.push(breakevenAfter(r,cs,.5));beOne.push(breakevenAfter(r,cs,1));}
    setSkipped({contractMismatch:mismatch,history});setScenarios([
      summarize('Frozen Plan',baselineResults,baselineResults),
      summarize('Full Exit at +0.50R',fullHalf,baselineResults),
      summarize('50% at +0.50R, Rest Frozen',partialHalf,baselineResults),
      summarize('BE after 5m Close ≥ +0.50R',beHalf,baselineResults),
      summarize('BE after 5m Close ≥ +1.00R',beOne,baselineResults),
    ]);
  }finally{setLoading(false)}};
  useEffect(()=>{if(resolved.length)void run();else setScenarios([])},[resolved.length]);
  const evidence=quality(scenarios[0]?.samples??0);
  return <Card><CardHeader title="Commodity Management Simulation" subtitle="Analytics-only comparison of frozen exits versus simple profit-protection rules on resolved MCX trades." action={<div className="flex gap-2 items-center"><Badge variant={evidence.variant}>{evidence.label}</Badge><Button size="sm" variant="ghost" onClick={()=>void run()} disabled={loading}>{loading?<RefreshCw size={13} className="inline mr-1 animate-spin"/>:<Activity size={13} className="inline mr-1"/>}Refresh</Button></div>}/><CardBody className="space-y-3"><div className="grid grid-cols-1 md:grid-cols-5 gap-2">{scenarios.map((s,index)=><div key={s.name} className="rounded-lg border border-slate-200 dark:border-slate-800 p-3"><div className="flex items-start justify-between gap-2"><p className="text-xs font-semibold">{s.name}</p>{index>0&&<Badge variant={s.pairedAvgDeltaR>0?'green':s.pairedAvgDeltaR<0?'red':'default'}>{s.pairedAvgDeltaR>=0?'+':''}{s.pairedAvgDeltaR.toFixed(2)}R vs base</Badge>}</div><p className="text-lg font-bold mt-1">{s.avgR.toFixed(2)}R</p><p className="text-[11px] text-slate-500">Avg R · Net {s.netR.toFixed(2)}R</p><p className="text-[11px] text-slate-500 mt-1">{s.samples} usable · {s.wins}W / {s.losses}L / {s.breakeven}BE</p>{index>0&&<p className="text-[11px] text-slate-500 mt-1">Paired {s.pairedSamples}: {s.improved} improved · {s.worsened} worse · {s.unchanged} same</p>}{s.excluded>0&&<p className="text-[11px] text-amber-600 mt-1">{s.excluded} ambiguous/excluded</p>}</div>)}</div><div className="rounded-lg border border-slate-200 dark:border-slate-800 p-3 text-xs text-slate-500"><b>Matched-sample safeguard:</b> the “vs base” figure compares only the same trades that are usable in both that scenario and the Frozen Plan. This prevents a scenario from looking better simply because difficult or ambiguous trades were excluded. Positive paired ΔR means the management rule improved average realised R on those matched trades.</div><div className="rounded-lg border border-slate-200 dark:border-slate-800 p-3 text-xs text-slate-500"><b>Simulation rules:</b> breakeven is armed only after a completed 5m candle closes at or beyond +0.50R/+1.00R, then applies from the next candle. The 50% partial scenario also requires a 5m close ≥ +0.50R. A full +0.50R exit uses OHLC touch and excludes candles that touch both +0.50R and SL because intrabar order is unknowable.</div>{(skipped.contractMismatch>0||skipped.history>0)&&<p className="text-[11px] text-slate-500">Excluded before simulation: {skipped.contractMismatch} rollover-contract mismatch · {skipped.history} insufficient retained 5m history.</p>}<p className="text-xs text-slate-500">No live commodity entry, stop, target or execution gate is changed by this panel. Use it only after enough resolved trades accumulate.</p></CardBody></Card>;
}
