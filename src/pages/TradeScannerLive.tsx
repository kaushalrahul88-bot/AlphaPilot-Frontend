...
function safetyGate(item: MtfScanItem, fno?: FnoScanResponse): SafetyGate {
  ...
  const known = LIVE_TIMEFRAMES
    .map(tf => String(frames?.[tf]?.market_structure ?? item.timeframes?.[tf]?.market_structure ?? '').toUpperCase())
    .filter(Boolean);
  const alignedCount = known.filter(s => direction === 'LONG' ? s === 'UPTREND' : s === 'DOWNTREND').length;
  const aligned = known.length < 2 || alignedCount >= 2;
  ...
  if (!aligned) return { pass: false, label: `WATCH ${action}`, reason: '5m/15m/1h direction agreement is below 2 timeframes.' };
...
export function TradeScannerLive({ onNavigate: _onNavigate }: { onNavigate: (p: PageKey) => void }) {
  ...
  return <div className="space-y-5">
    ...
    {!scanning && mode === 'single' && singleResult && (() => { const synthetic: MtfScanItem = { symbol: singleResult.symbol, status: singleResult.status, signal: singleResult.signal, direction: singleResult.technical?.direction, timeframes: singleResult.technical?.timeframes ?? singleResult.timeframes }; const gate = safetyGate(synthetic, singleResult); const direction = directionOf(synthetic, singleResult); const alpha = Number(singleResult.overall_alpha_score ?? 50); const underlyingRR = Number(singleResult.technical?.risk_reward ?? 0); const alphaPass = direction === 'SHORT' ? alpha <= 35 : alpha >= 65; const executionReady = singleResult.execution_ready === true && gate.pass && alphaPass && underlyingRR >= scannerFilters.minRiskReward; const option = (singleResult.recommended_option ?? {}) as any; const optionRR = optionRiskReward(option); return <Card><CardHeader title={`${singleResult.symbol} — ${executionReady ? optionAction(direction) : gate.pass ? 'REJECTED AFTER F&O' : gate.label}`} action={<Badge variant="blue">Alpha {alpha.toFixed(1)}/100</Badge>}/><CardBody><div className="grid grid-cols-2 md:grid-cols-5 gap-3"><Metric label="Status" value={executionReady ? 'SETUP' : 'WATCH'}/><Metric label="Technical" value={`${Number(singleResult.technical_score ?? 0).toFixed(1)}/100`}/><Metric label="F&O" value={`${Number(singleResult.fno_score ?? 0).toFixed(1)}/100`}/><Metric label="Option R:R" value={optionRR == null ? '—' : `${optionRR.toFixed(2)}:1`}/><Metric label="Execution" value={executionReady ? 'READY' : 'BLOCKED'}/></div>{option.contract_label && <div className="mt-4 rounded-lg border border-slate-200 dark:border-slate-800 p-3"><p className="font-semibold text-sm">{option.contract_label}</p><div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-3"><TradeMetric label="Lot Size" value={lotSize(option)}/><TradeMetric label="Amount Required (1 Lot)" value={amountRequired(option)}/><TradeMetric label="Option Entry" value={rupee(option.option_entry ?? option.premium)}/><TradeMetric label="Option Stop Loss" value={rupee(option.option_stop_loss)}/></div><div className="grid grid-cols-2 md:grid-cols-3 gap-3 mt-3"><TradeMetric label="Option Target 1" value={rupee(option.option_target1)}/><TradeMetric label="Option Target 2" value={rupee(option.option_target2)}/><TradeMetric label="Option R:R" value={optionRR == null ? '—' : `${optionRR.toFixed(2)} : 1`}/></div></div>}{gate.reason && <div className="mt-4 text-xs text-amber-600">• {gate.reason}</div>}{gate.pass && !alphaPass && <div className="mt-4 text-xs text-amber-600">• {direction === 'SHORT' ? `Final Alpha ${alpha.toFixed(1)} is above the BUY PE limit of 35.` : `Final Alpha ${alpha.toFixed(1)} is below the BUY CE threshold of 65.`}</div>}{gate.pass && underlyingRR < scannerFilters.minRiskReward && <div className="mt-4 text-xs text-amber-600">• Underlying R:R {underlyingRR.toFixed(2)} is below the required {scannerFilters.minRiskReward.toFixed(2)}.</div>}{Array.isArray(singleResult.execution_blockers) && singleResult.execution_blockers.length > 0 && <div className="mt-4 text-xs text-amber-600">{singleResult.execution_blockers.map((b: string, i: number) => <p key={i}>• {b}</p>)}</div>}</CardBody></Card>; })()}
  </div>;
}
