...
function ProbeCard({ symbol, state, onProbe }: { symbol: CommoditySymbol; state: ProbeState; onProbe: () => void }) {
  const r = state.result; const contract = r?.contract ?? {}; const tradingSymbol = contract.trading_symbol ?? contract.groww_symbol ?? '—'; const candleCount = Number(r?.candle_count ?? (Array.isArray(r?.candles) ? r?.candles.length : 0)); const checks = r?.checks ?? {}; const contractOk = Boolean(checks.contract ?? r?.contract); const quoteOk = Boolean(checks.quote ?? r?.quote_ok); const candlesOk = Boolean(checks.candles ?? r?.candles_ok ?? candleCount > 0); const ready = r?.ready_for_phase1 === true; const dte = daysToExpiry(contract.expiry_date ?? contract.expiry); const nearExpiry = dte != null && dte <= 3; const validatedLtp=Number((r as any)?.quote?.validation?.last_price); const probeErrors=normalizeProbeErrors(r?.errors);
  return <Card><CardHeader title={symbol} subtitle="Nearest non-expired MCX futures contract capability probe." action={<div className="flex items-center gap-2"><Badge variant={ready ? 'green' : r ? 'red' : 'default'}>{ready ? 'PHASE 1 READY' : r ? 'NOT READY' : 'NOT TESTED'}</Badge><Button size="sm" variant="ghost" onClick={onProbe} disabled={state.loading}>{state.loading ? <RefreshCw size={14} className="inline mr-1 animate-spin"/> : <Activity size={14} className="inline mr-1"/>}{state.loading ? 'Probing…' : 'Run Probe'}</Button></div>} /><CardBody className="space-y-4">{state.error&&<div className="rounded-lg border border-red-200 dark:border-red-900 bg-red-50/60 dark:bg-red-950/20 p-3 text-xs text-red-600">{state.error}</div>}{probeErrors.length>0&&<div className="rounded-lg border border-red-200 dark:border-red-900 bg-red-50/60 dark:bg-red-950/20 p-3"><p className="text-xs font-semibold text-red-600">Probe integrity failure</p>{probeErrors.map((e,i)=><p key={i} className="text-xs text-red-600 mt-1">• {e}</p>)}</div>}<div className="grid grid-cols-2 md:grid-cols-7 gap-3"><Metric label="Contract" value={tradingSymbol}/><Metric label="Expiry" value={String(contract.expiry_date ?? contract.expiry ?? '—')}/><Metric label="Days to Expiry" value={dte==null?'—':String(dte)}/><Metric label="Validated LTP" value={Number.isFinite(validatedLtp)?money(validatedLtp):'—'}/><Metric label="Exchange" value={String(contract.exchange ?? 'MCX')}/><Metric label="Segment" value={String(contract.segment ?? 'COMMODITY')}/><Metric label="5m Candles" value={Number.isFinite(candleCount)?String(candleCount):'—'}/></div>{nearExpiry&&<div className="rounded-lg border border-amber-200 dark:border-amber-900 p-3 text-xs text-amber-600"><AlertTriangle size={14} className="inline mr-1"/>Near-expiry contract: AlphaPilot currently selects the nearest non-expired MCX future. It does not switch early based on volume or open-interest migration, so verify live liquidity before execution.</div>}<div className="grid grid-cols-1 md:grid-cols-3 gap-2"><Check label="Contract resolution" ok={contractOk} tested={Boolean(r)}/><Check label="Validated Groww quote" ok={quoteOk} tested={Boolean(r)}/><Check label="Historical candles" ok={candlesOk} tested={Boolean(r)}/></div></CardBody></Card>;
}

function ScanCard({ symbol, state, enabled, onScan }: { symbol: CommoditySymbol; state: ScanState; enabled: boolean; onScan: () => void }) {
  const r=state.result; const action=r?.action ?? '—'; const ready=r?.execution_ready===true; const session=r?.market_session?.status ?? '—'; const dte=daysToExpiry(r?.contract?.expiry_date ?? r?.contract?.expiry); const nearExpiry=dte!=null&&dte<=3; const drift=Number(r?.entry_drift_r); const maxDrift=Number(r?.max_entry_drift_r??0.5); const priceGuard=r?.live_price_guard_passed;
  return <Card><CardHeader title={`${symbol} · Commodity Scanner`} subtitle="5m + 15m + 1h technical confluence with MCX session/freshness and live-LTP execution guards." action={<div className="flex items-center gap-2"><Badge variant={ready?'green':r?'blue':'default'}>{r ? (ready?'EXECUTION READY':r.status) : 'AWAITING SCAN'}</Badge><Button size="sm" variant="primary" onClick={onScan} disabled={!enabled||state.loading}>{state.loading?<RefreshCw size={14} className="inline mr-1 animate-spin"/>:<Activity size={14} className="inline mr-1"/>}{state.loading?'Scanning…':'Run MTF Scan'}</Button></div>} /><CardBody className="space-y-4">{!enabled&&!r&&<p className="text-xs text-slate-500">Run the Phase 1 probe first. Scanner stays disabled until contract, a validated positive live quote, and historical candles are confirmed.</p>}{state.error&&<div className="rounded-lg border border-red-200 dark:border-red-900 p-3 text-xs text-red-600">{state.error}</div>}{r&&<><div className="grid grid-cols-2 md:grid-cols-9 gap-3"><Metric label="Action" value={action}/><Metric label="Directional Strength" value={`${Number(r.alpha_score??0).toFixed(1)}/100`}/><Metric label="MCX Session" value={session}/><Metric label="DTE" value={dte==null?'—':String(dte)}/><Metric label="Signal Entry" value={money(r.entry)}/><Metric label="Live LTP" value={money(r.live_price)}/><Metric label="Entry Drift" value={Number.isFinite(drift)?`${drift.toFixed(2)}R / ${maxDrift.toFixed(2)}R max`:'—'}/><Metric label="Stop" value={money(r.stop_loss)}/><Metric label="R:R" value={r.risk_reward?`${Number(r.risk_reward).toFixed(2)}:1`:'—'}/></div>{priceGuard!=null&&<div className={`rounded-lg border p-3 text-xs ${priceGuard?'border-emerald-200 dark:border-emerald-900 text-emerald-600':'border-amber-200 dark:border-amber-900 text-amber-600'}`}>{priceGuard?<><CheckCircle2 size={14} className="inline mr-1"/>Live-price guard passed: current LTP remains within the permitted 0.50R distance from the candle-derived entry and has not crossed SL/T1.</>:<><AlertTriangle size={14} className="inline mr-1"/>Live-price guard failed. AlphaPilot will not mark this setup EXECUTION READY until a new scan produces acceptable live entry geometry.</>}</div>}{nearExpiry&&<div className="rounded-lg border border-amber-200 dark:border-amber-900 p-3 text-xs text-amber-600"><AlertTriangle size={14} className="inline mr-1"/>This setup is on a contract with {dte} day{dte===1?'':'s'} to expiry. READY still does not certify front-month liquidity versus the next contract.</div>}<div className="grid grid-cols-1 md:grid-cols-3 gap-2">{['5m','15m','1h'].map(tf=>{const row=r.timeframes?.[tf]??{};return <div key={tf} className="rounded-lg border border-slate-200 dark:border-slate-800 p-3"><div className="flex items-center justify-between gap-2"><b className="text-sm">{tf}</b><Badge variant={row.signal==='BUY'?'green':row.signal==='SELL'?'red':'default'}>{row.signal??'NO TRADE'}</Badge></div><p className="text-xs text-slate-500 mt-2">Raw Alpha {Number(row.alpha_score??0).toFixed(1)} · RSI {Number(row.rsi14??0).toFixed(1)}</p><p className="text-xs text-slate-500">{row.market_structure??'—'} · {row.fresh?'FRESH':'STALE/SNAPSHOT'}</p><p className="text-[11px] text-slate-500 mt-1">{formatTime(row.latest_candle_at)}</p></div>})}</div>{(r.blockers??[]).length>0&&<div className="rounded-lg border border-amber-200 dark:border-amber-900 p-3"><p className="text-xs font-semibold text-amber-600">Execution blocked</p>{r.blockers?.map((b,i)=><p key={i} className="text-xs text-slate-500 mt-1">• {b}</p>)}</div>}{ready&&<div className="rounded-lg border border-emerald-200 dark:border-emerald-900 p-3 text-xs text-emerald-600 font-semibold">All commodity scanner gates passed, including validated live-LTP drift. This futures setup will also be enrolled in Commodity Outcome Tracking and the MCX recorder will sync completed 5m candles.</div>}</>}</CardBody></Card>;
}
...
function money(value: unknown){const n=Number(value);return Number.isFinite(n)&&n>0?`₹${n.toFixed(2)}`:'—';}
...
function daysToExpiry(value: unknown){if(value==null||value==='')return null;const expiry=new Date(`${String(value).slice(0,10)}T23:59:59+05:30`);if(Number.isNaN(expiry.getTime()))return null;const now=new Date();return Math.max(0,Math.ceil((expiry.getTime()-now.getTime())/86_400_000));}
function normalizeProbeErrors(value:unknown):string[]{if(!value)return[];if(Array.isArray(value))return value.map(v=>typeof v==='string'?v:typeof v==='object'&&v?`${String((v as any).check??'probe')}: ${String((v as any).error??JSON.stringify(v))}`:String(v));if(typeof value==='object')return Object.entries(value as Record<string,unknown>).map(([k,v])=>`${k}: ${String(v)}`);return[String(value)];}
